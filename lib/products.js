// DEFAULT / FALLBACK menu for Tilanga Ji Ka Mashahur Peda Dukan.
//
// The live menu is edited from /admin and lives in MongoDB (see
// lib/db/products.server.js) — this file is the seed data
// (scripts/seed-db.mjs) and the offline fallback used whenever the
// database isn't configured or isn't reachable, so the site never breaks.
//
...
// inStock:     false -> "Sold out today" — no Add-to-cart even if deliverable.
// ONE item ships pan-India: Classic Khoya Peda. Every other peda flavour,
// the gift boxes, litti-chokha, fresh sweets and chai are dine-in only —
// visit the shop.
//
// pricePerKg: set only on the one item that's bulk-orderable.
...
export const products = [
  // ------------------------------------------------------------------ Peda
  {
    ...
    tagline: "The original, since 1975 — our only pan-India delivery item",
    description:
      "Our founding recipe — pure khoya, slow-cooked and hand-rolled the same way it was made when the shop started as a roadside stall in Sakaddi. No shortcuts, no additives. The one item on our menu that we pack and ship anywhere in India.",
    price: 349,
    mrp: 399,
    weight: "500g Box",
    pricePerKg: 560,
...
export function getDeliverableProducts() {
  return products.filter((p) => p.deliverable && p.inStock !== false);
}

// Loose peda that can be ordered by the kilo (Bulk Order page).
export function getBulkItems() {
  return products.filter((p) => p.deliverable && p.inStock !== false && typeof p.pricePerKg === "number");
}
